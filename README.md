...
The shared Run Scene Composer accepts optional PNG, JPEG, or WEBP global style and character references. SceneForge stores each through the existing sequence-reference boundary and analyzes a style reference through LiteLLM vision into one reusable `stylePrompt` segment. The segment is appended exactly once after Run resource-aware prompt formatting, including every Krea Preview and Final request. Illustrious-capable checkpoints continue to use their existing IPAdapter paths, and Anima retains its dedicated character path. Krea separates style-image conditioning from experimental ReID: style-only use keeps the existing verified `krea2_style_reference.safetensors` path, while an active prepared ReID reference pauses that adapter without deleting its state or analyzed prompt. ReID uses exactly one `krea2_reid_rank32.safetensors` model-only LoRA at 1.0, one `Krea2OstrisEditModelPatch` with `kv_cache=true`, and one prepared RGB image as `image1`; it never emits generic Krea character references or `image2`. Removing ReID restores a compatible selected style adapter. Preflight and queue-time validation fail closed before storage/upload or queueing when the exact local graph, ports, model metadata, or adapter file is unavailable. Workflow JSON stores only the selected prepared image's managed metadata, preparation choice, descriptor, and confirmation context—not raw uploads, alternate previews, data URLs, detector tensors/paths, or temporary ComfyUI names.
...
Krea formal planning and Preview stay locked to 8 steps, CFG 1, Euler, and simple scheduling. Style-only and prompt-only Final retain policy v3: Conservative is 4 steps at 0.12 denoise, Balanced is 4 at 0.18, and Strong is 6 at 0.28. Active ReID uses policy v4 and fixes Final to 8 steps while preserving the chosen redraw denoise, selected Preview seed, managed upscale, dimensions, prompt/resources, Detailers, and review. Repair never contains ReID LoRA, patch, descriptor, encoder, or reference image. Completed older Krea outputs remain displayable, while incomplete, missing-policy, or cross-policy older state cannot be retried or reused until reconfirmed. Illustrious, Anima, and fallback mappings remain unchanged.
