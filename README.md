...
Timeline workflow records are stored under `data/timeline-workflows/` by default. The active autosave record remains `active-workflow.json`; named workflow records are separate JSON files in the same directory. Records can hold either `single-image` Run workflows or `story-graph` workflows. Story Graph records may include Story Reference entity cards, reference asset metadata, candidate and approved reference URLs/filenames, user decisions, render recipes, location continuity modes, freeze-gate state, execution warnings, and generated-image references needed to restore progress. They must not contain API keys, `.env.local` secret values, generated image bytes or base64/data URLs, downloaded models, caches, logs, SQLite/resource database contents, or local ComfyUI temp payloads. Older or partial Story Reference records restore conservatively by staling affected reference, render, gate, and execution state instead of pretending generation is ready. Deleting a named workflow removes only that workflow JSON record and does not delete generated images or external assets referenced by the workflow.
