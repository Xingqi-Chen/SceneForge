...
The shared Run Scene Composer also accepts one optional PNG, JPEG, or WEBP global style reference. SceneForge stores it through the existing sequence-reference boundary and analyzes it through LiteLLM vision into one reusable `stylePrompt` segment. The segment is appended exactly once after Run resource-aware prompt formatting, including every Krea Preview and Final request. Illustrious-capable checkpoints may additionally enable IPAdapter with defaults `weight=0.45`, `start_at=0`, and `end_at=1`. A selected Krea 2 Turbo diffusion checkpoint may expose its Krea-specific reference adapter only after a no-queue local preflight verifies the Krea graph, inputs, and `krea2_style_reference.safetensors`; queue-time validation repeats that check. An explicitly selected Krea adapter remains selected while preflight is pending or unavailable, blocks generation, and requires explicit user opt-out before prompt-only generation can continue. Anima and unknown contexts remain visibly prompt-only. Pending, failed, or model-mismatched analysis blocks start, regeneration, and confirmation until the reference is retried, replaced, or removed. Workflow JSON stores only sanitized metadata, analysis context/status, and adapter settings—not image bytes, data URLs, or temporary ComfyUI input names.
...
Preview/Final review, repair diagnosis, and repair verification follow the same model boundary: ordinary Runs use `LITELLM_VISION_MODEL` with default-model fallback, while NSFW Runs require multimodal `LITELLM_NSFW_MODEL` and never fall back to an ordinary model.
