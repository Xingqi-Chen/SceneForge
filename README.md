...
### Krea 2 Turbo direct Run profile
...
Krea 2 Turbo queues one confirmed direct txt2img image only. Defaults are 1024×1024 with both dimensions aligned to 16, 8 steps, CFG 1, Euler/simple sampling, and batch size 1. It requires compatible local `UNETLoader`, `CLIPLoader` (`krea2`, `qwen3vl_4b_fp8_scaled.safetensors`), `VAELoader` (`qwen_image_vae.safetensors`), `KSampler`, `VAEDecode`, and `SaveImage` nodes; optional LoRAs use `LoraLoaderModelOnly`. SceneForge checks `object_info` and the exact required local model files before queueing, so an incomplete ComfyUI installation fails closed.

Krea direct Runs do not accept source images, style/IPAdapter references, ControlNet, FaceDetailer, HandDetailer, preview generation/scoring, Final redraw, review, or repair. Those timeline stages persist a visible structured “not applicable” result and do not make a ComfyUI or LLM call. Restoring a legacy Krea workflow clears those unsupported controls, invalidates downstream output, and requires a fresh confirmation before the single direct render.
