...
Krea requires compatible local `UNETLoader`, `CLIPLoader` (`krea2`, `qwen3vl_4b_fp8_scaled.safetensors`), `VAELoader` (`qwen_image_vae.safetensors`), `KSampler`, `VAEDecode`, `SaveImage`, and, for img2img, `LoadImage`, `ImageScale`, and `VAEEncode`; optional LoRAs use `LoraLoaderModelOnly`. FaceDetailer and HandDetailer are independent, user-controlled Final-only options: Preview always disables both. When either is enabled, SceneForge checks the full Detailer graph, required inputs, sampler, detector model, and Krea UNET/CLIP/VAE local context through `object_info` before queueing, so missing or incompatible support fails closed instead of being omitted. Style/IPAdapter references, ControlNet, Story, API/RAW, and inpaint remain unavailable. Until T42, Final review, repair, and repair verification persist structured `not-applicable` results and make no Vision, repair, or verification call. Completed legacy direct Krea outputs remain read-only with their original provenance; incomplete direct records stale and require fresh confirmation without invented Preview links.
