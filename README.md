...
Every Krea Run Style Advice request requires `parameterSuggestions.resolution` as exact integer `WIDTHxHEIGHT`, with both axes in 16–16384 and divisible by 16. For ordinary source img2img, SceneForge validates the source before calling Style Advice and requires the provider to return those exact dimensions without rounding, resizing, cropping, padding, stretching, substitution, or aspect-ratio change. For a positive-integer source-less recommendation with no saved parameters, SceneForge deterministically chooses the closest 16-aligned formal dimensions that can produce a non-upscaled, exact-aspect, 16-aligned Preview with longest edge at most 768; the parameter reason shows both values when they differ. Saved and source dimensions are never normalized, saved parameters continue to bypass Style Advice, and downstream validation remains fail-closed.
...
SceneForge exposes a server-side LiteLLM chat endpoint at `POST /api/llm/chat`. Existing AI features use this endpoint for prompt, tag, pose, diagnosis, enrichment, and recommendation flows. Single-image Run additionally uses narrow server-side planning boundaries and direct server adapters for the Responses-only calls described below; callers cannot select the transport.
...
The endpoint accepts `model`, `messages`, `temperature`, `maxTokens`, and optional `nsfw`. Run Scene Prompt Generation also sends `responseFormat`, but only the three exact server-authored Illustrious, Anima, and Krea 2 strict JSON Schema contracts are accepted; arbitrary schemas, `json_object`, mutations, and cross-purpose use are rejected before LiteLLM. Those exact structured single-image Scene Prompt requests are forwarded from `/api/llm/chat` to LiteLLM's `/v1/responses` API. The remaining single-image Run planning migrations use two dedicated server boundaries: Character Tags, Character Action, and conditional Parameter Recommendation Style Advice pass an exact node/purpose/prompt/temperature/token allowlist, while Run resource recommendation invokes the shared Civitai retrieval and validation service with an internal Responses completion executor. Preview Scoring, Final Review, eligible-pair Repair diagnosis, and Repair Verification call the same Responses client directly from their server-only timeline adapters on every initial and bounded retry attempt. The ordinary Civitai Library recommendation route remains on Chat Completions, as do Composer Suggest/Rewrite, Style Reference Analysis, Story, Editor, and every other excluded call.

For every authorized Responses call, SceneForge maps `maxTokens` to `max_output_tokens`, requests `stream: false`, sets `store: false`, and accepts exactly one completed assistant message whose content array contains exactly one non-empty `output_text` item. Any sibling, unknown, malformed, refusal, annotation-like, or additional text content item fails closed. Strict-schema Scene Prompt also maps its authorized schema to `text.format`; the migrated schema-free planning and Vision nodes omit `text.format` and keep their existing local JSON parsers and bounded retry policies. Multimodal Run requests preserve their existing message, text-part, and image order; text/image parts map to `input_text`/`input_image`, and the transient JPEG data URLs keep `detail: high`. A compatibility decoder tolerates a proxy that ignores `stream: false` only when it returns an official Responses event stream with one final `response.completed` event containing the full completed response. If that completed response is successful but explicitly contains `output: []`, the decoder may restore only one complete canonical assistant message from a uniquely indexed `response.output_item.done` event under the same single-content-item rule; missing, malformed, duplicate, or multiple candidate messages fail closed. Non-message terminal items and all delta, content-part, and output-text completion events are ignored. Standard SSE `id`/`retry` fields are ignored, and `[DONE]` is tolerated only as the final terminator after that completed response. Delta events are never assembled; top-level convenience text, Chat Completions SSE, early or non-final `[DONE]`, failed/incomplete/error events, missing completion, refusals, multiple completed assistant messages or text parts, and malformed output fail without a Chat Completions fallback.

Requests marked `nsfw` generally use `LITELLM_NSFW_MODEL` when it is configured. Single-image Run preview scoring is the fixed model-selection exception: ordinary, NSFW, and Krea scoring use only `LITELLM_DEFAULT_MODEL`, ignore `LITELLM_VISION_MODEL`, `LITELLM_NSFW_MODEL`, and any request-level model override, and preserve the true `nsfw` flag. Preview scoring fails with a recoverable configuration error before contacting LiteLLM when the default model is absent. The configured default model must support multimodal image input and have a content policy that permits the ordinary or NSFW images it is expected to score. Story Graph LLM planning nodes continue to use `LITELLM_NSFW_MODEL` for NSFW workflows, except `shot-dependency-graph`, `resource-plan`, and `parameter-plan`. Run and Story style-reference analysis reuse the `story-style-reference-analysis` purpose and fall back to `LITELLM_VISION_MODEL`, then `LITELLM_DEFAULT_MODEL`, when no explicit model is provided. Civitai semantic candidate retrieval requires `LITELLM_CIVITAI_EMBEDDING_MODEL` through LiteLLM's `/v1/embeddings` API during `npm run civitai:reindex-embeddings` and recommendation requests. Long Civitai source text is embedded in overlapping chunks, and recommendation ranking uses each resource's nearest chunk. Timeline model-resource and render-parameter recommendation nodes keep their purpose-specific models.

Final review of Preview/Final pairs, repair diagnosis, and repair verification keep their existing model boundary while using Responses: ordinary Runs use `LITELLM_VISION_MODEL` with default-model fallback, while NSFW Runs require multimodal `LITELLM_NSFW_MODEL` and never fall back to an ordinary model.
...
SceneForge is designed for local use. LLM request, response, and error records are written to split local JSONL files under `data/logs/llm/<category>/<YYYY-MM-DD>.jsonl` by default. Current categories include `chat`, `civitai-enrichment`, `civitai-recommendation`, `story-planning`, and `misc`. Ordinary Chat records keep full text prompts and model responses for diagnosis, while image data URLs are redacted before writing. Authorized Responses records use a stricter metadata-only policy: they record only the fixed call/purpose/status classifications, message/content counts and lengths, token counts, and allowlisted upstream/output-shape failure classifications. They do not record prompts, completion previews, model or routing values, schema bodies, data URLs, credentials, provider payloads, stack traces, or filesystem paths. The log directory is ignored by git, but users should still treat it as private local data.
