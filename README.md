...
12. User-selectable result display.
...
Preview/Final review follows the same model boundary: ordinary Runs use `LITELLM_VISION_MODEL` with default-model fallback, while NSFW Runs require multimodal `LITELLM_NSFW_MODEL` and never fall back to an ordinary model.
