...
Krea requires compatible local `UNETLoader`, `CLIPLoader` (`krea2`, `qwen3vl_4b_fp8_scaled.safetensors`), `VAELoader` (`qwen_image_vae.safetensors`), `KSampler`, `VAEDecode`, `SaveImage`, and, for img2img, `LoadImage`, `ImageScale`, and `VAEEncode`; optional LoRAs use `LoraLoaderModelOnly`. Prompt-only style references need no additional Krea nodes. The optional Krea reference adapter additionally requires `ComfyUI-Krea2-Ostris-Edit` (`TextEncodeKrea2OstrisEdit` and `Krea2OstrisEditModelPatch`) plus the exact local `krea2_style_reference.safetensors` LoRA; it is available only for compatible Krea 2 Turbo diffusion context and keeps timing fixed at `start_at=0`, `end_at=1`. SceneForge checks `object_info`, required inputs/samplers, and exact local model files before queueing, so incomplete ComfyUI installations fail closed without silently injecting or dropping an adapter. ControlNet, entity references, FaceDetailer, HandDetailer, Story, API/RAW, and inpaint remain unavailable. Until T42, Final review, repair, and repair verification persist structured `not-applicable` results and make no Vision, repair, or verification call. Completed legacy direct Krea outputs remain read-only with their original provenance; incomplete direct records stale and require fresh confirmation without invented Preview links.
