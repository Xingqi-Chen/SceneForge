...
SceneForge exposes a server-side LiteLLM chat endpoint at `POST /api/llm/chat`. Existing AI features use this endpoint for prompt, tag, pose, diagnosis, enrichment, and recommendation flows. Single-image Run additionally uses narrow server-side planning boundaries for the Responses-only calls described below; these routes authorize fixed Run node contracts rather than accepting a general client-selected transport.
