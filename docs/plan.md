...
| T-AN4 | #29 | Support Anima ControlNet, character references, and detailers | Editor ComfyUI Anima | In Progress | PASS | APPROVE | Depends on T-AN1 through T-AN3. First Anima release must support ControlNet, IPAdapter/character references, Face Detailer, and Hand Detailer. Detailers and add-ons must consume profile-provided model/clip/vae context instead of assuming CheckpointLoaderSimple. Unsupported or missing ComfyUI nodes/files must be caught before queueing with actionable errors. T-AN1 through T-AN4 define the first usable Anima release and are not optional later enhancements. Implementation validated; PR pending. |
