...
| T6 | #36 | Add centralized settings page | Agent Timeline MVP | Done | PASS | APPROVE | Central settings page for NSFW, path/status dashboard, Civitai path editing, and redacted integration status. Secrets remain server-only; full runtime secret editing is out of scope. |
