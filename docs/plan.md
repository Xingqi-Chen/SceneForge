...
Prioritize preparing issue-ready scope for `T-AN1`, the first temporary high-priority Anima workflow track. `T-AN1` through `T-AN4` are temporarily inserted ahead of `T6` and together define the first usable Anima release.
...
| T-AN1 | #23 | Add ComfyUI workflow profiles with Illustrious fallback and Anima txt2img | Editor ComfyUI Anima | In Progress | PASS | APPROVE | Temporary high-priority insertion before T6. Add workflow profile registry, preserve the existing Illustrious/default workflow as fallback, add Anima txt2img using UNETLoader + CLIPLoader + VAELoader + LoraLoader, keep preview metadata while capping preview steps, and make object_info validation profile-aware. Unknown or non-Anima diffusion models do not expose manual profile selection and fall back to Illustrious/default. PR #24 opened. |
| T-AN2 | TBD | Persist Anima model settings through /editor, style palette, Civitai resources, and saved params | Editor ComfyUI Anima | Todo | TBD | TBD | Depends on T-AN1. Auto-select Anima profile when selected Civitai checkpoint has baseModel Anima; strict Anima LoRA compatibility by baseModel Anima; CLIP/VAE settings should follow the existing Illustrious model settings pattern rather than being fixed; persist profile/baseModel/modelStorageKind and model settings in saved params, generated image history, sequence defaults/shots, and project serialization. |
| T-AN3 | TBD | Support Anima img2img, inpaint, high-res inpaint, and sequence inheritance | Editor ComfyUI Anima | Todo | TBD | TBD | Depends on T-AN1 and T-AN2. Reuse VAEEncode for img2img, VAEEncodeForInpaint for inpaint, and the existing high-res/local-region inpaint pipeline with Anima profile model context. Sequence shots must inherit workflow metadata and support txt2img, previous-shot img2img, and inpaint without falling back to Illustrious. |
| T-AN4 | TBD | Support Anima ControlNet, character references, and detailers | Editor ComfyUI Anima | Todo | TBD | TBD | Depends on T-AN1 through T-AN3. First Anima release must support ControlNet, IPAdapter/character references, Face Detailer, and Hand Detailer. Detailers and add-ons must consume profile-provided model/clip/vae context instead of assuming CheckpointLoaderSimple. Unsupported or missing ComfyUI nodes/files must be caught before queueing with actionable errors. T-AN1 through T-AN4 define the first usable Anima release and are not optional later enhancements. |
