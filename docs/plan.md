...
Open and validate the PR for `T2`. `T-CI` added the temporary repository CI setup, and `T1` has confirmed the backend contracts that the Agent single-image workflow will use before `/agent` UI or ComfyUI execution work starts.
...
| T2 | #4 | Add standalone Agent draft workflow | Agent MVP | In Progress | PASS | Self-reviewed | Depends on T1; `/agent` independent entry; LiteLLM creates editable single-image draft; user confirmation required before any ComfyUI call. |
