...
| T-AN3 | #27 | Support Anima img2img, inpaint, high-res inpaint, and sequence inheritance | Editor ComfyUI Anima | In Progress | PASS | APPROVE | Depends on T-AN1 and T-AN2. Reuse VAEEncode for img2img, VAEEncodeForInpaint for inpaint, and the existing high-res/local-region inpaint pipeline with Anima profile model context. Sequence shots must inherit workflow metadata and support txt2img, previous-shot img2img, and inpaint without falling back to Illustrious. Validation passed; reviewer approved; PR pending. |
