...
| T23A | #89 | Tighten Story Graph output contracts and raw debug boundaries | Story Graph Refinement | Done | PASS | APPROVE | Depends on T18 through T22. Implemented with T23B-D in PR #93; tester-agent and reviewer-agent passed. |
| T23B | #90 | Refine Anima Story prompts into natural visual clauses | Story Graph Refinement | Done | PASS | APPROVE | Depends on T18 through T22. Implemented in PR #93 with Story Anima prompt parts, LLM-owned shot-count decisions, render prompt normalization hardening, and prompt conflict handling. |
| T23C | #91 | Add source-image risk decisions for Story shot dependencies | Story Graph Refinement | Done | PASS | APPROVE | Depends on T18 through T22. Implemented in PR #93 with source-image risk metadata and execution-safe dependency handling for Story generation. |
| T23D | #92 | Redesign Story Visual Output around shot cards and prompt health | Story Graph Refinement | Done | PASS | APPROVE | Depends on T23A-C. Implemented in PR #93 with shot-card Visual summaries, prompt health, source-chain display, and Raw JSON debug boundaries. |
