...
Prioritize preparing issue-ready scope for `T-AN5`, the current temporary high-priority Anima editor prompt track. `T-AN1` through `T-AN5` are temporarily inserted ahead of `T6` and together define the first usable Anima release.
...
| T-AN5 | #31 | Add Anima prompt formatting for /editor | Editor ComfyUI Anima | In Progress | PASS | APPROVE | High-priority insertion before T6. Add Anima prompt output for editor preview, ComfyUI, Comic Sequence, and Selected Artist Strings. Enable automatically from Anima workflow profile/baseModel; keep Illustrious fallback unchanged. Use conservative safety behavior by omitting default `safe` when NSFW is enabled, and strictly reorder Anima prompt parts into the Anima-recommended order. PR #32 opened. |
