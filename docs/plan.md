...
| T24 | #98 | Add Story input style resource and parameter controls | Story Graph Workflow | Done | PASS | APPROVE | Depends on T20 through T23D. Migrate explicit checkpoint, LoRA, AI Style Advice inside the Parameters dialog, and supported ComfyUI parameter selection into `/story` start input only. Parameters can be saved only after explicit checkpoint selection; saved style resources bypass LLM resource planning and saved parameters bypass LLM parameter planning. Artist Strings, Subject Input, inpaint, ControlNet, imageCount, and sequence-specific settings remain out of scope. Merged PR #99; Issue #98 closed. |
| T25 | #102 | Add Story entity and shot-state planning contracts | Story Reference Workflow | Deferred | N/A | N/A | Closed issue #102 and unmerged PR #103 as not planned because the current Story Reference approach did not produce good enough results. Future work should be rescoped before implementation. |
| T26 | #104 | Add reference asset-plan and freeze-gate semantics | Story Reference Workflow | Deferred | N/A | N/A | Closed issue #104 and unmerged PR #105 as not planned because the current Story Reference approach did not produce good enough results. Future work should be rescoped before implementation. |
| T27 | #106 | Generate and upload Story reference plates | Story Reference Workflow | Deferred | N/A | N/A | Closed issue #106 and unmerged PR #107 as not planned because the current Story Reference approach did not produce good enough results. Future work should be rescoped before implementation. |
| T28 | #108 | Build Story Reference review workspace | Story Reference Workflow | Deferred | N/A | N/A | Closed issue #108 and unmerged PR #109 as not planned because the current Story Reference approach did not produce good enough results. Future work should be rescoped before implementation. |
...
| T32 | #118 | Add Story input FaceDetailer and HandDetailer controls | Story Graph Refinement | Done | PASS | APPROVE | Add explicit `/story` start input controls for FaceDetailer and HandDetailer. Detailer enablement and fine parameters are edited in the Story input Detailers section, independent of checkpoint/LoRA resource selection and the Story Parameters dialog. AI planning does not set detailers, final generation/regeneration use enabled configs, previews stay disabled, gate summaries show enabled state, and legacy workflow records default both detailers off. Merged PR #119; Issue #118 closed. |
| T33 | #122 | Add Story global style reference prompt and optional IPAdapter support | Story Graph Refinement | Done | PASS | APPROVE | Single global `/story` style reference for all shots: upload/analyze one image into style prompt, persist metadata/settings snapshot, merge prompt into final/regenerated shots, and inject sequence-style IPAdapter only for Illustrious-capable checkpoints with visible 0..1 controls/defaults. Anima/unsupported models are prompt-only; invalid upload/analysis/reference blocks start until retry/remove. Excludes entity/shot reference plates, character consistency workflow, and hidden image inheritance. Merged PR #123; Issue #122 closed. |
| T34 | #125 | Add Story Graph planning diagnostics and retry guard | Story Graph Refinement | Done | PASS | APPROVE | Categorized local LLM logs, Story planning request/response/error log context, no same-pass auto-run for errored workflow nodes, and Story prompt stability refinements. Merged PR #126; Issue #125 closed. |
