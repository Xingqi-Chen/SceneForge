...
| T9 | #42 | Add resource-aware final prompt formatting and local-only model selection | Agent Timeline MVP | In Progress | PASS | APPROVE | Depends on T7 and T8. Treat scene prompt as upstream semantic prompt draft, assemble the final ComfyUI prompt after selected local checkpoint and LoRAs are known, insert selected LoRA trainedWords from local metadata, format by checkpoint baseModel/profile, and prevent completed resource outputs from referencing checkpoint or LoRA resources outside the local candidate set. Follow-up preserves detailed ComfyUI object_info mismatch messages in step 9 when selected local Civitai resources are not available to the current ComfyUI instance. PR #43 opened. |
