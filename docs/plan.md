...
| T8 | #40 | Add confirmed single-image ComfyUI execution timeline nodes | Agent Timeline MVP | In Progress | PASS | APPROVE | Depends on T7. Timeline stops before generation. Only the user clicking start image generation can advance to ComfyUI execution and result display. MVP remains single-image only. |
