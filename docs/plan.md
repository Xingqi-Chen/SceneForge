...
| T-CI | N/A | Configure GitHub Actions CI | Repository Infrastructure | Done | PARTIAL | Self-reviewed | Temporary repository-bootstrap work requested directly; no GitHub Issue needed. Adds CI for install, lint, typecheck, test, and build. `git diff --check` passed; local npm validation was blocked by Windows sandbox process startup errors. |
