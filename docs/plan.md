...
Prioritize preparing issue-ready scope for `T7`, the checkpoint, LoRA, and parameter recommendation node track. `T-AN1` through `T-AN6` and `T6` are complete and together define the first usable Anima release plus the centralized settings entry point.
...
| T7 | #38 | Add checkpoint, LoRA, and parameter recommendation nodes | Agent Timeline MVP | In Review | PASS | APPROVE | Depends on T5 and T6. Reuse Civitai recommendation, selected-resource UI, and ComfyUI parameter controls. LLM must choose from local candidates, not invent unavailable resource names. PR #39 opened. |
