...
| T32 | #118 | Add Story input FaceDetailer and HandDetailer toggles | Story Graph Refinement | Ready | PASS | APPROVE | Add explicit `/story` start input checkboxes for FaceDetailer and HandDetailer. Checkbox state is the final execution switch, detailer fields come only from the current Story Parameters dialog or ComfyUI defaults, AI planning does not set detailers, final generation/regeneration use enabled configs, previews stay disabled, gate summaries show enabled state, and legacy workflow records default both detailers off. PR #119. |
