...
| T5 | #11 | Infer scene, character tags, action, and bind to 3D canvas | Agent Timeline MVP | In Progress | PASS | APPROVE | Depends on T3 and T4. First restores the timeline workbench to a usable desktop 3-column layout, then reuses existing LLM interfaces and 3D/stick-figure modules to infer scene prompt, main-character tags, action pose, and bind results to the 3D canvas. MVP supports one primary character. PR #12 opened. |
