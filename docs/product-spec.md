...
- Finals preserve the candidate seed, formal dimensions/steps/CFG/sampler/scheduler, checkpoint/LoRAs, prompts, style/IPAdapter context, and enabled Hand-before-Face Detailers. Internal Final denoise is 0.30 for Illustrious and 0.35 for Anima or unknown/default fallback without changing Composer or parameter-recommendation state. A fresh Final whose managed content hash is unchanged from its `preview-upscale` fallback is a recoverable failure. Detailed mode may override the selection with exactly K successful candidates.
- The versioned deterministic-resize and denoise policy is part of the signed confirmation contract. Missing or altered policy confirmation blocks new execution; already completed legacy results remain displayable. Partial failure and retry retain valid candidate fallbacks and successful Finals and execute only missing or invalid work.
