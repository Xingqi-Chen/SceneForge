...
- Empty-input Run Suggest requests six complete structured scene candidates, ranks them against at most 20 installation-local recent concept fingerprints, and chooses among the top three with 50/30/20 weights. Nonempty Suggest and Rewrite retain their existing behavior.
...
- Krea 2 keeps formal planning and Preview canonical at 8 steps, CFG 1, Euler, and simple scheduling across advice, saved parameters, confirmation, retry, and restore. Krea policy v3 resolves Final Conservative to 4 steps/0.12 denoise, Balanced to 4/0.18, and Strong to 6/0.28. Its 1024×1024 resolution is a fallback rather than a lock.
...
- Repair verification compares all available Preview/Final/Repair triples in one bounded high-detail request, with at most one safe schema repair. SceneForge derives the recommendation locally. A Repair is never auto-promoted and is selectable only when its managed output and successful verification pair both persist and reconcile safely.
