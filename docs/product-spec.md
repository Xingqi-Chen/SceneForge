...
The primary Krea sampler is fixed at 8 steps, CFG 1, Euler, and simple scheduling in the displayed parameter result, request preview, confirmation contract, Preview, Final, retry, and incomplete-workflow restore paths; conflicting AI advice, saved parameters, or persisted/client state cannot override those four values. This primary-sampler lock does not redefine independent FaceDetailer or HandDetailer sampler settings. Resolution defaults to 1024×1024 only when no compatible source, saved dimensions, or AI-advised resolution takes precedence. Composer source dimensions override saved and AI-advised dimensions, and Composer source denoise overrides saved denoise for source-img2img Preview generation. Invalid source, saved, or advised Krea dimensions fail the existing exact 16-pixel-alignment checks rather than being rounded or stretched. Final redraw denoise remains independently controlled by the existing Conservative/Balanced/Strong 0.35/0.45/0.55 policy. Illustrious and Anima behavior is unchanged.
...
- Krea 2 keeps its primary sampler canonical at 8 steps, CFG 1, Euler, and simple scheduling across advice, saved parameters, confirmation, Preview, Final, retry, and restore. Its 1024×1024 resolution is a fallback rather than a lock.
