...
The first Story Reference planning slice adds entity and shot-state contracts only. Story Bible props are first-class entities. Storyboard shots may expose structured `appearanceState`, `interactionState`, and `locationViewState`. The `entity-cards` planning node derives inspectable character, outfit, prop, and location cards from the Story Bible, storyboard shots, and character continuity graph. Missing or invalid structured references are preserved as recoverable planning errors that can be surfaced in consistency checks and Raw JSON instead of crashing the workflow or silently inventing references.
...
- Persistence must save reference artifact metadata, approval states, prompt-only fallback decisions, render recipes, freeze-gate state, execution warnings, and continuity modes without storing generated bytes, base64/data URLs, secrets, caches, logs, local model data, SQLite contents, temp payloads, or downloaded assets. Legacy or partial Story Reference records must restore inspectably and stale affected downstream state instead of pretending references exist or generation is ready.
