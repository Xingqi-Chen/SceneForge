...
- `/api/civitai-lora-library/imported-images/[id]/resource-selection`: read-only Run helper that validates the supplied current checkpoint id/type/base-model context and image-level base model, reads only persisted LoRA usages, and returns the unchanged checkpoint id, every ordered ready exact-base-model LoRA id, and safe skip reasons.
...
- `/api/agent-timeline/run-planning-response`: exact-contract Responses boundary for single-image Run Character Tags, Character Action, and conditional Style Advice only.
- `/api/agent-timeline/run-resource-recommendation`: single-image Run resource-recommendation boundary that preserves shared Civitai retrieval/ranking but injects a server-only Responses completion executor.
...
- Preserve the `/api/llm/chat` logging, validation, and model-selection semantics when wrapping existing LLM calls for LangGraph nodes. The generic route selects Responses only for an exact-whitelisted `stable-diffusion-prompt-generation` request with one of the three Run Scene Prompt schemas. Schema-free Run Character Tags, Character Action, and Style Advice use a separate route that checks the explicit node id plus the exact server-authored purpose, prompt fingerprint, payload shape, temperature, and token contract; it does not accept a transport flag or model override. Run resource recommendation similarly uses its own route and an internal Responses executor after the unchanged embedding/BM25/RRF candidate stage. Server-only Preview Scoring, Final Review, eligible-pair Repair diagnosis, and Repair Verification adapters invoke the shared Responses method directly for every initial and bounded retry attempt. The shared Civitai Library route and all excluded Composer, Story, Editor, and style-reference calls retain Chat Completions. All Responses calls set `store: false` and `stream: false`, omit `text.format` when schema-free, require one completed assistant message with exactly one non-empty `output_text` content item, reuse the fail-closed forced-SSE decoder, and never fall back to Chat. Multimodal mapping preserves message and part order, maps text/image parts to `input_text`/`input_image`, preserves data URL strings and `detail: high`, and does not change image preparation. Responses console and local logs are metadata-only and omit prompt/completion bodies, model routing, schema bodies, data URLs, credentials, provider payloads, stack traces, and filesystem paths; ordinary Chat logging is unchanged. Story Graph planning adapters should write local request, response, and error records with workflow id, story id, and node id context.
...
- The Run-only Civitai picker supports By resource and By image modes over the same Simple/Detailed Composer state. By image remains unavailable until the selected ready checkpoint preview has nonempty base-model metadata, then queries existing imported-image metadata by that image-level base model. On click, the server validates the supplied current checkpoint id/type/base-model context and the image-level base model, ignores all image checkpoint/model usages, reads only persisted LoRA usages, deduplicates them in stable order, and selects every ready LoRA with the exact checkpoint base model without the AI recommendation three-LoRA limit. A valid image with no eligible LoRAs succeeds with an empty LoRA list and clears prior LoRAs. Skipped LoRAs return safe names and reason codes/messages without download-status paths. Checkpoint/base-model changes reload the gallery and invalidate stale list or selection responses. Malformed responses or request failures leave the prior selection unchanged. Identical checkpoint plus identical ordered LoRA ids is a no-op before parameter/advice/stale/confirmation mutations; any ordered LoRA change atomically replaces the old ids through the existing Run resource-change path. Image selection does not use the prompt profile as its authority, adopt an image checkpoint, download, checksum-verify, call an external provider, apply image prompts/weights/parameters, change settings, or persist the image id.
...
Krea Run Style Advice request construction receives the trusted prompt profile explicitly rather than inferring it from resource prose. Its Krea-only system contract requires `parameterSuggestions.resolution` as integer `WIDTHxHEIGHT`, with each axis in 16–16384 inclusive and divisible by 16. Ordinary Krea source dimensions validate locally before the provider call and must be returned exactly without rounding, resizing, cropping, padding, stretching, substitution, or aspect-ratio changes. For source-less, unsaved advice with two parsed positive integers, Node 7 searches the complete feasible formal domain and minimizes squared relative axis error by comparing the exact common-denominator numerator. The Krea boundary admits a string to that normalization only when the whole resolution is an unsigned integer pair; exact negative axes remain negative for fail-closed validation, and malformed signed strings cannot be reinterpreted from an unsigned substring by the shared tolerant parser. Deterministic ties use the exact symmetric aspect error `|W*Ha-H*Wa|/(W*Ha+H*Wa)`, exact absolute pixel-area difference, non-enlargement, larger maximum eligible Preview area, smaller width, then smaller height. Feasibility and Node 9 share the same pure exact-aspect rule: formal axes are 16–16384 and divisible by 16, and the non-upscaled Preview must preserve aspect ratio with both axes divisible by 16 and longest edge at most 768. The existing reason records advised and normalized dimensions when they differ. Missing or invalid advice retains existing fallback/error behavior; source and saved dimensions are never normalized, saved parameters still bypass Style Advice, and downstream validation remains fail-closed.
