...
Krea 2 follows the same signed confirmation gate and ordinary staged Run execution. Its 1024×1024 / 8-step / CFG-1 / Euler-simple defaults apply to Preview and Final, with Krea-specific 16-pixel exact-aspect dimension checks. A ready global style reference is compiled exactly once into the Krea prompt before the parameter result is reviewed and remains byte-free in persistence; changing its analysis, context, mode, or adapter settings stales parameter planning and cancels confirmation. Prompt-only Krea generation remains usable when the local adapter preflight is unavailable. Selecting the adapter requires compatible Krea 2 Turbo context, exact timing 0→1, required custom nodes/ports, and the fixed local LoRA at both preflight and queue time; a failed queue-time recheck rejects the request before queueing rather than falling back silently. Krea source img2img uses Composer source denoise in preference to saved parameters; selected Preview candidates become server-side Lanczos3 `preview-upscale` artifacts and the sole Final img2img sources. The Krea Final policy maps Conservative/Balanced/Strong to 0.35/0.45/0.55 and is confirmation-bound like other profiles. Until T42, `final-review`, `final-repair`, and `repair-verification` return `{ status: "not-applicable", reason: "krea2-t42-unavailable", message }` before any Vision, ComfyUI repair, or verification call. Persistence reconciles new staged Krea Preview/score/fallback/Final linkage exactly like ordinary Runs; completed pre-T41 direct records remain read-only with direct provenance, while incomplete direct records are made stale and require a fresh confirmation without invented links.
