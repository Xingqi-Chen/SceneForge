...
- The Krea 2 profile is selected only when normalized Civitai base-model metadata identifies the Krea 2 family (for example `Krea 2` or `Krea-2 Turbo`) and model storage is `diffusion`; filenames and an asserted profile alone cannot select it. Requests that assert `workflowProfile: "krea2"` without that metadata are rejected. It builds `UNETLoader -> CLIPLoader(type=krea2) -> VAELoader -> optional LoraLoaderModelOnly -> EmptyLatentImage -> KSampler -> VAEDecode -> SaveImage`; it never uses `CheckpointLoaderSimple`, img2img, preview, IPAdapter, ControlNet, or Detailer nodes. The profile pins `qwen3vl_4b_fp8_scaled.safetensors`, `qwen_image_vae.safetensors`, and UNET dtype `default`, and `object_info` validation must verify required nodes, inputs, sampler options, optional model-only LoRA support, and exact local model-file availability before queueing.
...
Run single-image definition v4 executes `generation-gate → preview-execution → preview-scoring → comfyui-execution → final-review → final-repair → repair-verification → result-display`. K is 1-4 for ordinary txt2img and img2img Runs and maps to candidate pools 4/4/6/8. Each ordinary preview is a batch-size-1 request with a materialized base seed plus candidate index, disabled Detailers, formal CFG/sampler/scheduler/context retained, and `min(formal steps, 20)`. Initial fixed-seed execution uses the formal seed as its base. After workflow sanitization and confirmation-fingerprint verification, only the server handling that explicit `preview-execution` retry passes a transient adapter authorization to advance the retained `baseSeed` by `candidateCount`; no retry marker is accepted from or written to workflow persistence. Base and per-candidate seed addition wrap through zero at `Number.MAX_SAFE_INTEGER`, so consecutive authorized retries move through non-overlapping windows. Completed-result inspection does not advance, and normal stale regeneration after prompt/resource changes plus reconfirmation restarts from the newly confirmed formal seed. Random policy always materializes a fresh base. The transient authorization is request-local and outside the persisted workflow and confirmation contract. Ordinary preview dimensions use longest-edge 768. Dimensions already within 768 remain unchanged and are never upscaled. Larger integer inputs are reduced by their greatest common divisor, then use the largest common multiplier that keeps both axes 8-pixel aligned and within 768, preserving the formal aspect ratio exactly; for example, 832×1216 becomes 520×760. If no exact aligned multiplier fits an extreme ratio, request construction fails closed with an actionable validation error instead of independently rounding or stretching either axis. Source-img2img previews use the original source and Composer denoise.

Krea 2 follows the same signed confirmation gate but is a direct one-request branch. It uses one 16-pixel-aligned txt2img result with defaults 1024×1024, 8 steps, CFG 1, Euler/simple, denoise 1, and batch size 1. `preview-execution`, `preview-scoring`, `final-review`, `final-repair`, and `repair-verification` persist only the closed structured result `{ status: "not-applicable", reason: "krea2-direct-txt2img", message }`; they must not call ComfyUI or LiteLLM. The sole `comfyui-execution` call retrieves `object_info`, validates the direct request, queues exactly once, stores the output, and Result display consumes that direct final without Preview linkage, final policy, fallback, or repair state. Krea persistence accepts this structured stage result and skips ordinary Preview/Final linkage reconciliation. Restoring a Krea workflow removes source, style/IPAdapter, Detailer, and automatic-repair settings before any continuation, so legacy state cannot reach queueing.
