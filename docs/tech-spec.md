...
- T53 adds one optional byte-free Run character-reference image, separately stored from img2img and the analyzed global style reference. Its normalized `0..1` strength defaults to `0.8`, is shared by Simple and Detailed Composer modes, and every identity/role/effective-strength edit stales parameter recommendation plus all downstream execution and cancels confirmation. Default/Illustrious and Anima Preview/Final requests inject it through the existing IPAdapter path with strict `object_info` validation; an explicitly selected character image never silently downgrades to prompt-only. Confirmation captures ordered role identities and effective values in a safe immutable reference context; Final and Repair persistence/digests use that context rather than later mutable Composer state. Legacy no-reference records remain usable.
- Krea never sends generic `characterReferences`. Every active Krea reference is conditioned only through one verified `Krea2OstrisEdit` path and fixed `krea2_style_reference.safetensors`: character-only uses `image1=character`, style-only uses `image1=style`, and dual-reference uses `image1=style,image2=character`, with exactly one model-only loader and model patch. Krea timing is fixed at `0→1`; one persisted Shared Krea reference strength (`0.8` default) governs every active Krea role, and non-Krea reference controls remain unchanged. Krea preflight and queue-time validation require compatible Krea 2 Turbo diffusion context, `TextEncodeKrea2OstrisEdit`, `Krea2OstrisEditModelPatch`, the selected ports (including `image2` for dual input), `LoraLoaderModelOnly`, and the exact adapter file before upload or queue. Missing support blocks the selected reference without fallback. Img2img source upload remains independent; batch size, source dimensions/denoise precedence, and Hand-before-Face Detailer workflow order are unchanged.
...
- Run `scene-prompt` requests keep the `stable-diffusion-prompt-generation` purpose and carry one server-authored OpenAI-compatible `response_format` with `type: json_schema`, `strict: true`, and the stable profile-specific name `sceneforge_run_scene_prompt_<profile>_v1`. The Illustrious, Anima, and Krea 2 schemas close the root and every nested object, require all shared scene-context fields, type `style`, `camera`, and `lighting` as arrays of required `label`/`prompt` objects, and allow an empty string-array `negativeSuggestions`. Each schema permits and requires only its selected profile section. Illustrious and Anima require their current section keys as string arrays; Krea requires its six model-authored prose strings and omits locally owned `selectedLoraTriggerWords`. The prompt includes one `JSON.parse`-valid example for the selected profile only. The shared request validator accepts only the three exact whitelisted schemas for this purpose and rejects malformed, modified, or cross-purpose response formats before LiteLLM. The client forwards the accepted contract as `response_format` while retaining `stream: true`; provider rejection uses the existing sanitized error path without retry, repair, probing, `json_object` fallback, provider fallback, or model/routing changes. Logs retain only response-format type, stable name, and strictness rather than duplicating the schema.
...
- The Krea 2 profile is selected only when normalized Civitai base-model metadata identifies the Krea 2 family (for example `Krea 2` or `Krea-2 Turbo`) and model storage is `diffusion`; filenames and an asserted profile alone cannot select it. Requests that assert `workflowProfile: "krea2"` without that metadata are rejected. Its standard graph is `UNETLoader -> CLIPLoader(type=krea2) -> VAELoader -> optional LoraLoaderModelOnly -> (EmptyLatentImage or LoadImage -> ImageScale -> VAEEncode) -> KSampler -> VAEDecode -> SaveImage`, preserving the fixed model context for both Preview and Final img2img. A verified Run reference branch uses one `LoraLoaderModelOnly(krea2_style_reference.safetensors) -> Krea2OstrisEditModelPatch` and `TextEncodeKrea2OstrisEdit` conditioning route: `image1` is style when active (otherwise character), and dual role input adds character as `image2`; negative conditioning uses the same Krea encoder without a reference image. Generic IPAdapter and generic `characterReferences` are prohibited for Krea. FaceDetailer and HandDetailer are independent additions for Final and compatible Repair requests. They run Hand-before-Face and consume the selected graph's model connection, including the Krea reference patch when present on Final, plus the same Krea CLIP/VAE context. The profile pins `qwen3vl_4b_fp8_scaled.safetensors`, `qwen_image_vae.safetensors`, and UNET dtype `default`; `object_info` validation must verify every selected-path node and required input, selected dual-reference ports, KSampler/Detailer sampler options, detector models, optional model-only LoRA support, and exact local model-file availability before queueing.
