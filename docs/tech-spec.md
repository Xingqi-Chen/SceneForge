...
- The Krea 2 profile is selected only when normalized Civitai base-model metadata identifies the Krea 2 family (for example `Krea 2` or `Krea-2 Turbo`) and model storage is `diffusion`; filenames and an asserted profile alone cannot select it. Requests that assert `workflowProfile: "krea2"` without that metadata are rejected. It builds `UNETLoader -> CLIPLoader(type=krea2) -> VAELoader -> optional LoraLoaderModelOnly -> (EmptyLatentImage or LoadImage -> ImageScale -> VAEEncode) -> KSampler -> VAEDecode -> SaveImage`, preserving the fixed model context for both Preview and Final img2img. It never uses `CheckpointLoaderSimple`, IPAdapter, or ControlNet. FaceDetailer and HandDetailer are independent Final-only additions: Preview always disables both, while enabled Krea Final Detailers run in Hand-before-Face order using the same Krea UNET/CLIP/VAE context. Before any such Final queues, strict `object_info` preflight must validate the complete generated graph, every selected-path node and required input, KSampler/Detailer sampler settings, detector models, optional model-only LoRA support, and exact local Krea UNET/CLIP/VAE files. The profile pins `qwen3vl_4b_fp8_scaled.safetensors`, `qwen_image_vae.safetensors`, and UNET dtype `default`. T44 does not change the T42 Repair contract; future T42 Repair work may remain compatible with this Final-only Detailer path.
...
Krea 2 follows the same signed confirmation gate and ordinary staged Run execution. Its 1024×1024 / 8-step / CFG-1 / Euler-simple defaults apply to Preview and Final, with Krea-specific 16-pixel exact-aspect dimension checks. Krea source img2img uses Composer source denoise in preference to saved parameters; selected Preview candidates become server-side Lanczos3 `preview-upscale` artifacts and the sole Final img2img sources. The Krea Final policy maps Conservative/Balanced/Strong to 0.35/0.45/0.55 and is confirmation-bound like other profiles. FaceDetailer and HandDetailer remain disabled for Krea Preview execution and may be enabled independently only on Krea Final execution after the strict Krea context preflight. Until T42, `final-review`, `final-repair`, and `repair-verification` return `{ status: "not-applicable", reason: "krea2-t42-unavailable", message }` before any Vision, ComfyUI repair, or verification call; T44 does not alter that contract, and future T42 Repair work may remain compatible with Krea Final Detailers. Persistence reconciles new staged Krea Preview/score/fallback/Final linkage exactly like ordinary Runs; completed pre-T41 direct records remain read-only with direct provenance, while incomplete direct records are made stale and require a fresh confirmation without invented links.
...
Final execution resolves the confirmed formal dimensions, reads each selected managed Preview, verifies exact aspect compatibility, applies EXIF orientation, and uses Sharp Lanczos3 to produce an exact-size PNG without crop, padding, independent-axis rounding, or stretch. The resulting candidate-linked managed `preview-upscale` is stored before ComfyUI queueing and is the sole Final img2img source; Illustrious style-reference upload remains separate. Final policy v2 stores only the `conservative | balanced | strong` enum in `RunSceneInputSettingsSnapshot`; missing, unknown, or numeric values sanitize to default `balanced`. Its complete preset-by-family mapping is Illustrious 0.30/0.40/0.50 and Anima/fallback 0.35/0.45/0.55. The resolved policy version, resize mode, preset, family, and denoise are bound into the signed confirmation, gate summary, execution result, and each Final record without mutating source denoise or the formal parameter result. Final requests otherwise restore formal dimensions/steps/CFG/sampler/scheduler/resources/prompts/style context and Hand-before-Face Detailers and reuse the candidate seed. A preset-only mutation preserves valid Preview execution/scoring/selection/seed state, stales only `comfyui-execution` and `result-display`, blocks the gate for reconfirmation, and resumes from Final. Retry reuse requires exact resolved-policy equality, so same-preset partial work is reusable and cross-preset Finals are not. History extraction accepts only the queued workflow's declared output node. A fresh Final with the same managed content hash as its `preview-upscale` becomes a recoverable no-op error. Final records preserve Preview, fallback, Final, seed, candidate, and unique global-rank linkage, so a Detailed K=2 selection may legitimately retain ranks 1 and 3. Completed policy-v1 results remain read-only displayable; incomplete or confirmed policy-v1 state must be reconfirmed. Persistence accepts only normalized managed references and policy/linkage metadata, never bytes, data URLs, absolute paths, masks, full ComfyUI workflows, secrets, or raw service responses.
