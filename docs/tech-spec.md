...
- T55 supersedes T53 for Krea character references. Krea never sends generic `characterReferences`: its only character path is ReID with exact `krea2_reid_rank32.safetensors`, strength `1.0`, `Krea2OstrisEditModelPatch(kv_cache=true)`, and one prepared positive `image1`. `image2` and dual style/character graphs are forbidden. A separate style-only Krea reference may still use `krea2_style_reference.safetensors`, but active ReID pauses that transport without deleting its state and retains the analyzed style prompt exactly once. Krea ReID preflight and queue-time validation require authoritative Krea diffusion metadata, exact nodes/ports/file, fixed sampling, and the generated-graph invariant before upload or queue; missing support blocks without fallback. Img2img source upload remains independent, and Repair strips all ReID transport and descriptors.
- T55 supersedes the former Krea character-only/dual-image branch. `CharacterReferenceSnapshot.kind` distinguishes prepared `krea2-reid` state from generic/legacy character snapshots; a missing kind is generic and can never migrate into ReID. The confirmed ReID context is version 2 and authorizes exactly one character-role managed reference at strength 1. Krea style-only context remains the existing version-1 `krea2-ostris` style role. Active ReID suppresses style transport only; stored style metadata, adapter selection, and the exact-once analyzed prompt remain intact.
...
- The Krea 2 profile is selected only when normalized Civitai base-model metadata identifies the Krea 2 family (for example `Krea 2` or `Krea-2 Turbo`) and model storage is `diffusion`; filenames and an asserted profile alone cannot select it. Requests that assert `workflowProfile: "krea2"` without that metadata are rejected. Its standard graph is `UNETLoader -> CLIPLoader(type=krea2) -> VAELoader -> optional LoraLoaderModelOnly -> (EmptyLatentImage or LoadImage -> ImageScale -> VAEEncode) -> KSampler -> VAEDecode -> SaveImage`, preserving the fixed model context for both Preview and Final img2img. Style-only generation may use `LoraLoaderModelOnly(krea2_style_reference.safetensors) -> Krea2OstrisEditModelPatch` with style as positive `image1`. Krea character conditioning never reuses that style encoder contract: it uses only the mutually exclusive ReID graph described below, with one prepared character `image1`; `image2`, generic IPAdapter, generic `characterReferences`, and dual style/character conditioning are prohibited. FaceDetailer and HandDetailer are independent additions for Final and compatible Repair requests. They run Hand-before-Face and consume the selected graph's model connection plus the same Krea CLIP/VAE context. The profile pins `qwen3vl_4b_fp8_scaled.safetensors`, `qwen_image_vae.safetensors`, and UNET dtype `default`; `object_info` validation must verify every selected-path node and required input, KSampler/Detailer sampler options, detector models, optional model-only LoRA support, and exact local model-file availability before queueing.
- The ReID graph is mutually exclusive with Krea style transport and contains exactly one `LoraLoaderModelOnly(krea2_reid_rank32.safetensors,strength_model=1)`, one `Krea2OstrisEditModelPatch(kv_cache=true)`, two `TextEncodeKrea2OstrisEdit` nodes, and exactly one prepared `LoadImage` connected only as positive `image1`; no `image2` is emitted. ReID Preview and Final use 8 steps, CFG 1, Euler/simple. Request validation, `object_info` validation, and a post-build generated-graph audit all run before queueing and require authoritative Krea diffusion metadata plus the exact LoRA option. Repair construction deletes both ReID transport and descriptor and inpaint validation rejects either field, so Repair cannot contain the ReID LoRA, patch, encoder, or image. Prompt-only/style-only Krea, Illustrious, and Anima graphs retain their existing behavior.
...
Without active ReID, Krea policy version 3 is isolated from the unchanged ordinary Final policy v2. `resolveTimelineFinalGenerationPolicy` returns v3 only for prompt-only or style-only family `krea2` Runs, with Conservative `{ steps: 4, denoise: 0.12 }`, Balanced `{ steps: 4, denoise: 0.18 }`, and Strong `{ steps: 6, denoise: 0.28 }`; active ReID returns policy v4 with Final fixed to 8 steps, and Illustrious, Anima, and fallback continue to resolve their v2 denoise mappings and formal step behavior. Krea formal planning and Preview remain 8 steps, CFG 1, Euler, and simple. Final overwrites only base-sampler steps and denoise, reuses the selected candidate seed, and reads the candidate's policy-version-matched managed `preview-upscale`. Prompts, resources, style-reference binding, Detailers, source Preview denoise, review, and Repair semantics are unchanged.

When the confirmed reference context is version-2 `krea2-reid`, the resolver returns Krea policy v4 and fixes Final steps to 8; preset denoise, candidate seed, exact managed source and dimensions, prompt/resources, Detailers, and review remain unchanged. Removing ReID returns the Run to the compatible v3 policy after reconfirmation. Repair remains on its bounded source-img2img/inpaint contract and receives no ReID descriptor or graph nodes.
